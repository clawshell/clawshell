...
pub fn chat_completions_to_responses(body: &[u8]) -> Result<Vec<u8>, TranslateError> {
    ...
    let obj = req.as_object().ok_or(TranslateError::MissingField("root object"))?;
...
pub fn responses_to_chat_completion(body: &[u8]) -> Result<Vec<u8>, TranslateError> {
    ...
    let obj = resp.as_object().ok_or(TranslateError::MissingField("root object"))?;
...
mod tests {
    ...
    fn test_chat_to_responses_strips_unsupported() {
        ...
        for field in STRIP_FIELDS {
            assert!(parsed.get(*field).is_none(), "field '{}' should be stripped", field);
        }
    }
